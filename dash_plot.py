...
class Plot:
    ...
    def setup(self, options):
        if options.enableDB:
            ...
            client.users.authenticate(options.User, options.Password)
            db = client.users
            df = pd.DataFrame()
            # Data retrieval from mongoDB
            for item in db.ESF.find({'job_number': {'$regex': 'T2*'}}):
                se = pd.Series(item).T
                df = df.append(se, ignore_index=True)
            self.df = df.drop('_id', axis=1)
        else:
            self.df = pd.read_csv(options.InputData).drop(
                ['Unnamed: 0'], axis=1
            )
    ...
    def dash_plot(self):
        ...
        def update_graph(plot_type_value, x_axis_column_name,
                         y_axis_column_name,
                         z_axis_column_name, colour_column_value,
                         range_column_value,
                         range_slider_value):
            ...
            # 3D scatter plot
                ...
                return {
                    ...
                    'layout': go.Layout(
                        height=800,
                        scene={'xaxis': {'title': x_axis_column_name,
                                         'zeroline': True},
                               'yaxis': {'title': y_axis_column_name,
                                         'zeroline': True},
                               'zaxis': {'title': z_axis_column_name,
                                         'zeroline': True}}
                    ),
                    'margin': {'l': 40, 'b': 40, 't': 10, 'r': 0}
                }
